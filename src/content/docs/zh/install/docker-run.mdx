---
title: '安装: Docker run'
description: '任务专用说明： 安装: Docker run: `DATABASE_URL` · `discvault-postgres` · `6080:5000` · `/api/next/health`.'
...
完成此页面专用的流程： **安装: Docker run**. 此流程使用 `DATABASE_URL` · `discvault-postgres` · `6080:5000` · `/api/next/health`.

## 适用范围

本页面仅适用于 **安装: Docker run** (Docker). 通道与数据边界为： DiscVault · `ghcr.io/helmerznl/discvault:latest`; PostgreSQL API MCP; `6080` → API `5000`; DiscVault · `ghcr.io/helmerznl/discvault:beta`; PostgreSQL API MCP; `6080` → API `5000`. 本页面专用检查点： `DATABASE_URL` · `discvault-postgres` · `6080:5000` · `/api/next/health`.
...
1. **记录:** `DISCVAULT_IMAGE` · `:latest` `:beta`
2. **创建:** `--env-file` · `DATABASE_URL` · 仅所有者可访问的文件权限 (0600)
3. **启动:** `discvault-postgres` → `discvault-api` → MCP
4. **验证:** `6080:5000` · `GET /api/next/health`
...
仅在以下条件下接受结果： **安装: Docker run** 必须确认以下所有结果： HTTP `2xx` · `/api/next/health`; `discvault-postgres`. 本页面专用检查点： `DATABASE_URL` · `discvault-postgres` · `6080:5000` · `/api/next/health`.
...
:::caution
安全恢复边界： **安装: Docker run** 在启动或继续任何操作前必须执行： Docker Compose; 匹配的备份集 · PostgreSQL `/data`. 本页面专用检查点： `DATABASE_URL` · `discvault-postgres` · `6080:5000` · `/api/next/health`.

- Docker Compose
- 匹配的备份集 · PostgreSQL `/data`
:::
