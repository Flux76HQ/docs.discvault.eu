...
description: 'Deploy DiscVault 26 beta from the canonical moving Compose and environment files.'
...
Deploy DiscVault 26 beta from the canonical moving Compose and environment files.
...
- official files from `release/v26-beta`
- Compose project `discvault_next_deploy`
- host `6180` → API container `5000`
...
1. **Install:** `docker-compose.yml` · `.env.example` · `release/v26-beta`
2. **Create:** `/opt/discvault-next/.env` · owner-only file permissions (0600) · random secrets
...
4. **Verify:** `DISCVAULT_NEXT_IMAGE=ghcr.io/helmerznl/discvault:beta` · no `:dev`
5. **Run:** `docker compose -p discvault_next_deploy config --quiet` · pull · up
