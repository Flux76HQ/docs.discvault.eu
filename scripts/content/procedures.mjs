...
curl --version`,
  ...
  'install/index': {
    markers: ['`:latest`', '`:beta`', '`DISCVAULT_IMAGE`', 'PostgreSQL + `/data`'],
    ...
    steps: [
      ...
      step('record', '`DISCVAULT_IMAGE`', 'image digest', term('matchingBackup')),
    ],
    blocks: [
      routes(
        [
    ...
    outcomes: [item(term('selectedChannel')), item('PostgreSQL + `/data`', 'both channels')],
    safety: [item(term('matchingBackup')), item(term('releaseNotes'))],
  },
  'install/docker-run': {
...
docker inspect discvault-next-api-1 --format '{{range .Mounts}}{{.Source}} -> {{.Destination}}{{println}}{{end}}'`,
  ...
  'install/reverse-proxy-passkeys': {
    markers: ['`RP_ORIGINS`', '`RP_ID`', '`6080`', '`/api/next/health`'],
    prerequisites: [item('public DNS'), item('TLS certificate'), item('reverse proxy')],
    channels: releaseChannels(
...
curl --fail https://discvault.example.com/api/next/health`,
    ...
    outcomes: [item('HTTPS health', 'HTTP `2xx`'), item('passkey', '`RP_ID` + public origin')],
    safety: [item(term('proxyRecovery'))],
  },
...
curl --fail http://localhost:6080/api/next/auth/status`,
  ...
  'update/update': {
    markers: ['`:latest`', '`:beta`', '`DISCVAULT_IMAGE`', '`--force-recreate`'],
    prerequisites: [item(term('matchingBackup')), item(term('releaseNotes'))],
    channels: releaseChannels(
    ...
    steps: [
      step('verify', 'backup manifest', '`PREVIOUS_IMAGE=@sha256:…`'),
      step('choose', '`DISCVAULT_IMAGE`', '`:latest` / `:beta`'),
      step('run', 'Compose pull + `--force-recreate`', 'without deleting data'),
      step('compare', term('updateAccepted')),
    ],
    blocks: [
      code('stable', `export DISCVAULT_IMAGE=${imageChannels.stable.image}`),
      code('beta', `export DISCVAULT_IMAGE=${imageChannels.beta.image}`),
...
trap - EXIT`,
    ...
    outcomes: [item(term('updateAccepted'))],
    safety: [item(route('update/rollback'), term('matchingBackup')), item(term('releaseNotes'))],
  },
  'update/rollback': {
    markers: [
      '`PREVIOUS_IMAGE`',
...
curl --fail http://localhost:6080/api/next/migration/status`,
  ...
  'configure/auth-rbac': {
    markers: ['`/api/next/auth/status`', '`/api/next/auth/rbac`', 'WebAuthn', '`48 h`'],
    prerequisites: [
      item(term('ownerAccess')),
      item('HTTPS + passkey'),
      item(term('recoveryMethod')),
    ],
    channels: releaseChannels(item('WebAuthn', 'owner / administrator / editor / fan / viewer')),
    steps: [
...
curl --fail http://localhost:6080/api/next/auth/rbac`,
    ...
    outcomes: [item(term('authAccepted'))],
    safety: [item(term('authRecovery'))],
  },
  'configure/plugins-metadata': {
    markers: [
...
curl --fail http://localhost:6080/api/next/metadata/plugins`,
  ...
  'pwa/library-search': {
    markers: ['PWA', '`watchlist`', '`history`', 'RBAC'],
    prerequisites: [item(term('signedInAccount')), item('collection permission')],
    channels: releaseChannels(item('PWA', 'RBAC', 'account permissions')),
    steps: [
    ...
    blocks: [],
    outcomes: [item(ui('librarySearch')), item(ui('libraryFilter')), item(ui('titleWatchlist'))],
    safety: [item(term('libraryRecovery'))],
  },
  'pwa/offline': {
