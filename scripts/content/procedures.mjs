  ...
  'install/reverse-proxy-passkeys': {
    ...
    channels: {
      ...
      beta: [
        ...
        item('OIDC', '`/api/next/auth/oidc/callback`', route('configure/oidc')),
    ...
    steps: [
      ...
      step('configure', route('configure/oidc'), '`/api/next/auth/oidc/callback`'),
...
curl --fail http://localhost:6180/api/next/health`,
    ...
    outcomes: [
      ...
      item('OIDC', '`/api/next/auth/oidc/callback`'),
...
curl --fail http://localhost:6080/api/next/migration/status`,
  ...
  'configure/index': {
    ...
    steps: [
      ...
      step('choose', route('configure/oidc'), 'OIDC + Pocket ID'),
    ...
    blocks: [
      routes(
        ...
        ['configure/oidc', 'DiscVault 26.9.117 beta · OIDC · Pocket ID · account linking'],
  ...
  'configure/auth-rbac': {
    ...
    channels: {
      ...
      beta: [
        item(imageChannels.beta.label, `\`${imageChannels.beta.image}\``),
        item('WebAuthn + optional Legacy Authentication + OIDC'),
    ...
    steps: [
      ...
      step('compare', route('configure/oidc'), term('oidcPurpose')),
...
curl --fail http://localhost:6180/api/next/auth/rbac`,
  ...
  'configure/oidc': {
    ...
    prerequisites: [
      item(term('ownerAccess'), term('recoveryMethod')),
      item(route('install/reverse-proxy-passkeys'), term('fqdnSecureContext')),
      item(term('oidcProviderSetup')),
    ],
    notices: [
      {
        heading: term('oidcPrivateBackchannelHeading'),
        label: term('availableBetaLabel'),
        paragraphs: [term('oidcPrivateBackchannelBoundary')],
      },
    ],
    channels: {
      beta: [
        item(imageChannels.beta.label, '`26.9.117+`'),
        item(term('oidcPurpose')),
        item(term('oidcAccountSafety')),
      ],
    },
    steps: [
      ...
      step('configure', '`DISCVAULT_OIDC_PROVIDER_NAME`', 'optional label', 'default `OIDC`'),
      step(
        'choose',
        '`DISCVAULT_OIDC_INSECURE_BACKCHANNEL_ORIGINS`',
        '`localhost`',
        ...
        '`.internal`',
        term('oidcPrivateBackchannelBoundary'),
      ),
      step('start', '`next-api`', '`--force-recreate`'),
      step('test', term('oidcPurpose'), term('oidcAccountSafety')),
      step('inspect', '`auth.oidc_failed`', term('oidcAuditSafety')),
    ],
    blocks: [
      code(
        'beta',
...
curl --fail https://discvault.example.com/api/next/auth/status`,
    ...
    outcomes: [
      item(term('oidcPurpose')),
      item(term('oidcAccountSafety')),
      item(term('oidcAuditSafety')),
    ],
    safety: [
      item(term('oidcSecretRollback')),
      item(term('oidcPrivateBackchannelBoundary')),
      item(term('oidcAccountSafety')),
    ],
  },
  'configure/plugins-metadata': {
...
curl --fail 'http://localhost:6080/api/next/digital-items?limit=200'`,
  ...
  'troubleshooting/index': {
    ...
    steps: [
      ...
      step('inspect', route('configure/oidc'), '`auth.oidc_failed`'),
...
${composeCommand} logs --tail=200 next-api next-worker next-mcp postgres`,
  ...
  'reference/index': {
    ...
    steps: [
      ...
      step('record', route('configure/oidc'), '`DISCVAULT_OIDC_ISSUER`'),
